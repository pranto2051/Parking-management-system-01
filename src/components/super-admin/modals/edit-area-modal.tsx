...
export function EditAreaModal({ isOpen, onClose, area }: EditAreaModalProps) {
  const [formData, setFormData] = useState({ name: "", code: "", district_id: "" });
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    if (area) {
      setFormData({ name: area.name, code: area.code, district_id: area.district_id });
    }
  }, [area]);

  const handleSubmit = async () => {
    if (!formData.name || !formData.code || !formData.district_id) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative z-50 w-full max-w-md bg-white rounded-3xl border border-slate-200 shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h2 className="text-xl font-bold text-slate-800">Edit Area</h2>
            ...
            <div className="p-6 space-y-5">
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1">Area Name</label>
                <Input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="h-12 rounded-xl border-slate-200" />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1">Area Code</label>
              ...
              <div className="space-y-2">
                ...
                <Select value={formData.district_id} onChange={(e) => setFormData({ ...formData, district_id: e.target.value })} className="h-12 rounded-xl border-slate-200">
